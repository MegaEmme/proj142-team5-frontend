...
const BlogCard = ({ blog }) => {
    ...
    return (
        <div className=" mb-5 p-3 card"> {/* Colonne Bootstrap per layout responsive */}
            <div className="h-100 shadow-sm d-flex flex-row"> {/* Card Bootstrap con altezza fissa e ombra */}
                ...
                <div className="card-body d-flex flex-column">
                    ...
                    <h4 className="card-subtitle mb-2 text-muted my-2">{blog.category}</h4>
                    ...
                    <div className="d-flex justify-content-between align-items-center my-2 px-5">
                        <small className="text-muted">di {blog.author}</small>
                        <small className="text-muted">{blog.date}</small>
                    </div>
                    <Link to={`/post/${blog.id}`} className="text-center">
                        <button className="btn btn-success my-4 align-items-center px-4">Leggi di più</button>
                    </Link>
                </div>
            </div>
        </div>
